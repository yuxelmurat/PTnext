...
export async function currentSeasonHistory(root, current) {
  const weeks = {};
...
export async function weeklyReports(root, current, snapshots, previous = []) {
  ...
  for (let round = 1; round <= snapshots.at(-1).round; round++) {
    ...
    if (completed) {
      Object.assign(report, compareReport(snapshot.projection.table, await readWeeklyStandings(root, current, round)));
